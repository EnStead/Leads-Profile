...
try {
  ...
  if (res.adminData.success) {
    ...
    showToast(res.adminData.message);
    navigate("/admin/dashboard", { replace: true });
  } else {
    showToast(res.adminData?.message || "Login failed", "error");
  }
} catch (err) {
  ...
  return (
    ...
    >
      ...
      <p className='text-brand-darkblue text-[10px] lg:text-base  font-medium absolute top-4 right-8 ' >
        © Leads Profile 2025
      </p>

      <ToastPop message={toastMsg} type={toastType} onClose={() => setToastMsg("")} />
    </section>
  )
}
