...
import PasswordDawn from "../../../assets/PasswordDawn.jpg";
import PasswordEvening from "../../../assets/PasswordEvening.jpg";
...
const ChangePassword = () => {
  ...
  const [bgImage, setBgImage] = useState(PasswordBg);

  useEffect(() => {
    const hour = new Date().getHours();
    if (hour >= 5 && hour < 8) {
      setBgImage(PasswordDawn);
    } else if (hour >= 8 && hour < 17) {
      setBgImage(PasswordBg);
    } else if (hour >= 17 && hour < 21) {
      setBgImage(PasswordEvening);
    } else {
  ...
  return (
    <section
      className="min-h-screen relative py-30"
      style={{
    ...
    >
      <div className="flex justify-center items-center h-full">
        <div className="border w-lg border-brand-white bg-brand-white/30 backdrop-blur-[3px] rounded-2xl pb-10 pt-24 px-10 relative">
          ...
          <p className="text-brand-subtext text-center text mb-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col relative">
              <label
                className={`mb-1 font-medium text-lg ${form.password?.trim() !== "" ? "text-brand-blackish" : "text-brand-body"}`}
              >
                New Password
              </label>
              <input
            ...
            <div className="flex flex-col relative">
              <label
                className={`mb-1 font-medium text-lg ${form.confirmpass?.trim() !== "" ? "text-brand-blackish" : "text-brand-body"}`}
              >
                Confirm Password
              </label>
              <input
            ...
            <div className="flex flex-col">
              <label className="mb-1 font-medium text-lg text-brand-body">
                Reset Token
              </label>
              <input
